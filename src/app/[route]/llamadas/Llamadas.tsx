...
const calls = [
  ...
  },
  {
    ...
    startDate: 1743526000000,
    endDate:   1743526200000,
    attendees: ["Usuario", "Representante de Ventas"],
    transcript:
  ...
  },
  {
    ...
    startDate: 1743600000000,
    endDate:   1743600300000,
    attendees: ["Cliente", "Soporte Técnico"],
    transcript:
  ...
  },
  {
    ...
    startDate: 1743700000000,
    endDate:   1743700200000,
    attendees: ["Cliente", "Banco"],
    transcript:
  ...
  },
  {
    ...
    startDate: 1743800000000,
    endDate:   1743800500000,
    attendees: ["Cliente", "Transportista"],
    transcript:
  ...
  },
  {
    ...
    startDate: 1743900000000,
    endDate:   1743900300000,
    attendees: ["Candidato", "RRHH"],
    transcript:
  ...
  },
  {
    ...
    startDate: 1744000000000,
    endDate:   1744000400000,
    attendees: ["Cliente", "Finanzas"],
    transcript:
  ...
  },
  {
    ...
    startDate: 1744100000000,
    endDate:   1744100450000,
    attendees: ["Usuario", "Soporte Técnico"],
    transcript:
  ...
  },
  {
    ...
    startDate: 1744200000000,
    endDate:   1744200200000,
    attendees: ["Cliente", "Servicio al Cliente"],
    transcript:
  ...
  },
  {
    ...
    startDate: 1744300000000,
    endDate:   1744300300000,
    attendees: ["Cliente", "Transportista"],
    transcript:
...
type CallDetails = {
  ...
  ociAnalysis: {
    ...
    relevantAspects: Array<{ text: string; sentiment: string; confidence: number }>;
  };
};
...
export default function Llamadas() {
  ...
  const [tusLlamadas, setTusLlamadas] = useState<Call[]>(calls.slice(0, 5));
  const [assignedLlamadas, setAssignedLlamadas] = useState<Call[]>(calls.slice(5));

  const handleSelectCall = async (id: string) => {
    setLoading(true);
    const callFound = calls.find((c) => c.id === id)!;
    setSelectedCall(callFound);
    setSubpages([callFound.title]);
    const data = await analyzeCall(callFound.transcript);
    setCallDetails(data);
    setLoading(false);
  };
  ...
  const handleAcceptAssigned = (id: string) => {
    const call = assignedLlamadas.find((c) => c.id === id);
    if (!call) return;
    setTusLlamadas((prev) => [...prev, call]);
    setAssignedLlamadas((prev) => prev.filter((c) => c.id !== id));
  };
                ...
                <div key={call.id} className={styles.assignedCall}>
                  ...
                  <div className={styles.callActions}>
                    <button
                      ...
                      onClick={() => handleDeleteAssigned(call.id)}
                    >
                      Eliminar
                    </button>
                    <button
                      className={styles.acceptButton}
                      onClick={() => handleAcceptAssigned(call.id)}
                    >
                      Aceptar
                    </button>
                  </div>
                ...
                <div className={styles.callDetailsContentContainer}>
                  ...
                  <div style={{ display: "flex", gap: "2rem" }}>
                    <div className={styles.callDetailsContent}>
                      ...
                      <p className={styles.text}>
                        {parseDate(selectedCall?.startDate || 0)}
                      </p>
                    </div>
                    <div className={styles.callDetailsContent}>
                      ...
                      <p className={styles.text}>
                        {calcDuration(
                          selectedCall?.startDate || 0,
                          selectedCall?.endDate || 0
                        )}
                      </p>
                    </div>
                  </div>
